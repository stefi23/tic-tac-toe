...
const gamePlay = async (player) => {
  ...
  while (!board.gameOver()) {
    if (player === "X") {
      ...
      if (
        board.isPlayerCloseToWinning("0") === false &&
        board.isPlayerCloseToWinning("X") === false
      ) {
        if (board.board[5] === "") {
          position = 5;
        } else {
      ...
      if (board.isPlayerCloseToWinning("0")) {
        position = board.isPlayerCloseToWinning("0");
      } else if (board.isPlayerCloseToWinning("X")) {
        position = board.isPlayerCloseToWinning("X");
      }
